    ...
    def load_next_files(self):
        ...
        for i_file, filename in enumerate(self.current_files):
            ...
            if self.token_reco_cfg is not None:
                ...
                ak_x_particles, _, ak_jet_type_labels, p4 = read_jetclass_file(
                    filename,
                ...
                # Get the preprocessing cuts
                pp_dict_cuts = {
                feat_name: {
                    criterion: self.feature_dict[feat_name].get(criterion)
            ...
            unmasked_particles = ak.zip({"part_pt": p4.pt, "part_eta": p4.eta, "part_etarel": p4.deltaeta(p4_jet), "part_phi": p4.phi, "part_phirel": p4.deltaphi(p4_jet), "mass": p4.mass}, with_name="Momentum4D")
            ...
            cluster = fj.ClusterSequence(masked_particles, jet_def)
            jets = cluster.jets()
            jets_out = ak.firsts(cluster.inclusive_jets())
            jets_ak = ak.Array(
                {
                "part_pt": jets.pt, 
                "part_etarel": jets.deltaeta(jets_out), 
                "part_phirel": jets.deltaphi(jets_out)
                }, 
            with_name="Momentum4D",
            )

            # Continue as usual
            ak_x_jets = ak_select_and_preprocess(jets_ak, self.feature_dict)
            ak_x_jets_padded, ak_mask_jets = ak_pad(
                ak_x_jets, self.pad_length, return_mask=True
            )
            np_x_jets_padded = ak_to_np_stack(
                ak_x_jets_padded, names=self.particle_features_list
            )
            np_mask_jets = ak.to_numpy(ak_mask_jets)
            np_jet_type_labels = ak_to_np_stack(ak_jet_type_labels, names=self.labels_to_load)

            # add the data to the lists
            self.part_data_list.append(torch.tensor(np_x_jets_padded))
            self.mask_data_list.append(torch.tensor(np_mask_jets, dtype=torch.bool))
